      ...
      {tables.map((table) => (
        <div key={table.id || table.number} className="aspect-square bg-white rounded-3xl p-4 shadow-sm border border-gray-100 relative group flex flex-col justify-between">
          ...
          <div className="flex justify-between items-start">
            <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${getTableStatus(table.number) === 'Occupied'
          ...
          <div className="flex justify-center">
            <button
              ...
              className="p-2 bg-gray-50 rounded-xl text-gray-400 hover:bg-brand-dark hover:text-white transition-colors"
              title="View QR Code"
            >
              <QrCode size={18} />
            </button>
